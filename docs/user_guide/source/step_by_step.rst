...
.. note::
    ...
    * The access and secret keys required by the object store are automatically generated by the ``nlds init`` command.  The previous workflow of generating the object store access and secret keys is no longer required.

    * In addition, ``nlds init`` command will now fill in all of the required values in the ``~/.nlds-config`` file, including the user and group.  This makes the setting up of the NLDS client much more user friendly.
