...
.. note::
    ...
    In December 2025, a more user friendly method of setting up the NLDS client on 
...
.. note::
    As of December 2025, the access and secret keys required by the object store are automatically generated by the ``nlds init`` command. The previous workflow of generating the object store access and secret keys is no longer required.
...
.. note::
   In December 2025, NLDS moved to using PyPi for hosting the NLDS client, which makes installation much more straightforward for the user, as ``pip install nlds-client`` can be used, rather than the long GitHub URL as previously.
...
.. note::
    In December 2025, the NLDS client was improved so that the ``nlds init`` command will now fill in all of the required values in the ``~/.nlds-config`` file.  This makes the setting up of the NLDS client much more user friendly.
...
.. .. note::
..     If you are taking part in the NLDS beta testing program, then you will have to edit a line in the ``object_storage`` part of the config.  This is for the ``tenancy``, which should be set to ``nlds-staging-o.s3.jc.rl.ac.uk``.  i.e.:
